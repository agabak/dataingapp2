...
namespace Datingapp.API.Controllers
{
    ...
    public class PhotosController: ControllerBase
    {
        ...
        {
            var userPhotoFromRepo = await _repo.GetPhoto(id);
            var photo = _mapper.Map<PhotoForReturnDto>(userPhotoFromRepo);

            return  Ok(photo);
        }

         [HttpPost]
        public async Task<IActionResult> AddPhotoForUser(int userId, [FromForm]CreatePhotoDto createPhotoDto)
        {
    ...
    public async Task<IActionResult> SetMainPhoto(int userId, int id) 
    {
           ...
           if(!user.Photos.Any(p => p.Id == id))
            ...
            if(photoFromRepo.IsMain) {
                return BadRequest("Its Main photo");
            }
            
            //  I have the user object already, there is no need to go back to the serve
    ...
    public async Task<IActionResult> DeletePhoto(int userId, int id)
    {
          if(userId != int.Parse(User.FindFirst(ClaimTypes.NameIdentifier).Value))
           ...
           if(!user.Photos.Any(p => p.Id == id))
            ...
            if(photo.IsMain) {
                return BadRequest("Its main photo, you can not delete it");
            }
        ...
        return BadRequest("Enable to delete the photo");
    }
 }
}
